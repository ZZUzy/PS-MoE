...
def setup_logging(data_dir):
    """设置日志系统"""
    log_dir = os.path.join(data_dir, "graph_generation_logs")
...
def log_statistics(stats, log_file):
    """记录统计信息"""
    logging.info("\n" + "=" * 80)
    logging.info("GRAPH GENERATION STATISTICS")
    logging.info("=" * 80)
    logging.info(f"Total complexes processed: {stats['total_complexes']}")
    logging.info(f"Successful graphs generated: {stats['successful_graphs']}")
    logging.info(f"Skipped complexes: {stats['skipped_complexes']}")
    logging.info(f"Failed complexes: {stats['failed_complexes']}")
    logging.info(f"Success rate: {(stats['successful_graphs'] / max(1, stats['total_complexes'])) * 100:.2f}%")
...
def arg_parser():
    ...
    parser.add_argument('--data_dir', type=str, default=".\Complex", help='Path to the data directory containing all proteins(PDB) and ligands (SDF)')
    parser.add_argument('--save_dir', type=str, default=".\ComplexGraph",
                        help='Path to the data directory containing all proteins (PDB files)')
    parser.add_argument('--Ankh_dir', type=str, default=".\Ahkn",
                        help='Path to the Ankh embedding')
    parser.add_argument('--ESM_dir', type=str, default=".\ESM2",
                        help='Path to the ESM embedding')
    parser.add_argument('--ChemBERTa_dir', type=str, default=".\ChemBERTa",
                        help='Path to the ChemBERTa embedding')
...
def Node_direct(X):  # 旋转不变的特征表示
    ...
    # 计算局部坐标系的基础向量
    u = F.normalize(A_n - A_ca, dim=0)
    v = F.normalize(A_ca - A_c, dim=0)
    b = F.normalize(u - v, dim=0)
    n = F.normalize(torch.cross(u, v), dim=0)

    # 构建完整的局部坐标系（3x3矩阵）
    local_frame = torch.stack([b, n, torch.cross(b, n)], dim=1)  # 形状 [3, 3]

    # 计算所有点相对于 A_ca 的向量，并转换到局部坐标系
    t = F.normalize(X - A_ca, dim=1)  # 形状 [3, 3]
    node_direct = torch.matmul(t, local_frame)  # 形状 [3, 3]

    return node_direct.flatten().unsqueeze(0)  # 展平为 9 维向量


def Node_rbf(X, rbf_num, D_min=0., D_max=20.): # 将原子间的距离编码为连续的向量表示
    # Distance radial basis function
    D_count = rbf_num
    D_mu = torch.linspace(D_min, D_max, D_count, device=X.device)
    D_sigma = (D_max - D_min) / D_count
    rel_list = [[0, 0, 0, 1, 1, 2], [1, 2, 3, 2, 3, 3]]  # 对应元素顺序
    D = torch.norm(X[rel_list[0]] - X[rel_list[1]], dim=-1)  # [B, N, 15]
    D_expand = torch.unsqueeze(D, -1)
    D_out = torch.exp(-((D_expand - D_mu) / D_sigma) ** 2).flatten(-2)
    return D_out.unsqueeze(0)
...
def get_atom_features(mol, all_atoms, padding_len=0):
    ...
    for atom in mol.GetAtoms():
        ...
        padding = [0 for n in range(padding_len)] # 保持特征向量维度一致 配体原子：原子特征(40维) + padding(20维0) = 60维 蛋白质残基：padding(40维0) + 氨基酸身份(20维) = 60维
        symbol = atom.GetSymbol() # 获取原子的化学元素符号
...
def main():
    ...
    # 设置日志系统
    stats, log_file = setup_logging(data_dir)
    logging.info(f"Starting graph generation for data in: {data_dir}")
    logging.info(f"Graphs will be saved to: {save_dir}")
    logging.info(f"Protein embeddings: {protein_embeddings}")
    logging.info(f"Ligand embeddings: {ligand_embeddings}")
    logging.info(f"Replace existing graphs: {replace_existing_graphs}")
    logging.info(f"Add masternode: {masternode}")
    logging.info("-" * 80)
    # --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    # PREPROCESSING
    # --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for i, protein in enumerate(proteins):
        ...
        complex_id = protein.name.split('.pdb')[0]
        stats['total_complexes'] += 1
        logging.info(f"[{i + 1}/{len(proteins)}] Processing complex: {complex_id}")
        ...
        try:
            ...
            # PARSING OF PROTEIN PDB TO GENERATE COORDINATE MATRIX 解析蛋白质PDB生成坐标矩阵
            # -----------------------------------------------------
            with open(protein_path) as pdbfile:
                protein_dict = parse_pdb(parser, id, pdbfile) # PDFfile存储的蛋白质信息 转化为 字典
            ...
            for chain in protein_dict:
                chain_comp = protein_dict[chain]['composition'] # 属性判断
                ...
                if chain_comp == [True, False] or chain_comp == [True, True]:
                    ...
                    for residue in protein_dict[chain]['aa_residues']:
                        ...
                        memb = [residue_idx for atom in res_dict['atom_indeces']] # 残基到原子的映射
            ...
            for l, ligand_mol in enumerate(ligands):
                ...
                if several_ligands:
                    ligand_id = f'{id}_L{l + 1:05}'
                    print(f'--- Ligand {l+1:05}', end=': ', flush=True)
                    id_with_lig = f'{id}_L{l+1:05}'
                    save_path = os.path.join(save_dir, f"{id_with_lig}_graph.pth")
                    logging.info(f"  Processing ligand {l + 1}/{len(ligands)}: {ligand_id}")
                else:
                    ligand_id = id
                    print(f'--- Ligand 1', end=': ', flush=True)
                    id_with_lig = id
                    save_path = os.path.join(save_dir, f"{id_with_lig}_graph.pth")
                    logging.info(f"  Processing ligand 1/1: {ligand_id}")
                ...
                # # COMPUTE CONNECTIVITY BETWEEN LIGAND AND PROTEIN ATOMS 计算配体和蛋白质原子之间的连通性
                # # -----------------------------------------------------
                max_len = 5
                diff = protein_atomcoords[np.newaxis, :, :] - ligand_atomcoords[:, np.newaxis, :] # 计算所有蛋白质原子与所有配体原子之间的坐标差值
                pairwise_distances = np.linalg.norm(diff, axis=2) # 欧几里得范数 就是原子间的距离
                close = pairwise_distances <= max_len + 1
                # 将距离小于5埃的这些原子索引映射到残基编号并去除重复编号
                ...
                if protein_embeddings:
                    ...
                    for j, emb in enumerate(protein_embeddings):
                        if emb == 'ankh_base':
                            search_pattern = os.path.join(Ankh_dir, f"{id}_{emb}*.pt")
                            matching_files = glob.glob(search_pattern)
                        else:
                            search_pattern = os.path.join(ESM_dir, f"{id}_{emb}*.pt")
                            matching_files = glob.glob(search_pattern)
                ...
                if ligand_embeddings is not None:
                    ...
                    for j, emb in enumerate(ligand_embeddings):
                        search_pattern = os.path.join(ChemBERTa_dir, f"{id}_{emb}*L{l+1:05}.pt")
                        matching_files = glob.glob(search_pattern)
                        if len(matching_files) == 1:
                ...
                #------------------------------------------------------------------------------------------
                x = get_atom_features(ligand_mol, all_atoms, padding_len=len(amino_acids)) # 得到给定分子的原子特征。
                padding = np.zeros([x.shape[0], 17], dtype=np.float32)
                ...
                for residue, resname in zip(connections_res_num, connections_res_name):
                    ...
                    if resname in amino_acids:
                        ...
                        merged_coords = np.vstack([n_coords, ca_coords, c_coords])
                        node_dir = Node_direct(torch.tensor(merged_coords, dtype=torch.float))

                        rbf_num = 8

                        o_idx = residues_dict[residue]['atoms'].index('O')
                        o_coords = residues_dict[residue]['coords'][o_idx]
                        merged_coords = np.vstack([n_coords, ca_coords, c_coords, o_coords])
                        node_rbf = Node_rbf(torch.tensor(merged_coords, dtype=torch.float), rbf_num)

                        aa_identity = torch.tensor(aa_identity, dtype=torch.float32)
                        features = torch.cat((aa_identity, node_dir, node_rbf), 1)
                ...
                mapping = {key: value for key, value in zip(connections_res_num, new_indeces)} #残基和原子的映射
        ...
        # If a fatal error occurs, break the loop to end the script
            ...
            logging.error(f"    ✗ Fatal error for complex {complex_id}: {e}")
            ...
            reason = f"Unexpected error ({error_type}): {str(e)}"
            if reason not in stats['failed_reasons']:
                stats['failed_reasons'][reason] = 0
            stats['failed_reasons'][reason] += 1
            logging.error(f"    ✗ Unexpected error for complex {complex_id}: {e}", exc_info=True)
            print(f"Unexpected error: {e}", flush=True)
            continue

    log_statistics(stats, log_file)
